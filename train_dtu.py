...
parser = argparse.ArgumentParser(description='A PyTorch Implementation of MVSMamba')
...
parser.add_argument('--trainpath', default='F:\Data\DTU\DTU_Dataset\dtu_training', help='train datapath')
...
parser.add_argument('--image_size', type=str, default=(1024, 1280), help='training image size')
parser.add_argument("--n_views", type=int, default=5, help='number of input views')
...
parser.add_argument('--depth_inter_r', type=str, default="2.0,1.0,1.0,0.5", help='depth_intervals_ratio')
...
parser.add_argument('--group_cor_dim', type=str, default="8,8,8,8", help='group correlation dim')
...
if __name__ == '__main__':
    ...
    if (not is_distributed) or (dist.get_rank() == 0):
        ...
        print('Number of model parameters: {:.2f}M'.format(total_params))
    ...
    if args.dataset.startswith('dtu'):
        train_dataset = MVSDataset(args.trainpath, args.trainlist, "train", args.n_views, args.interval_scale, rt=args.rt,  use_raw_train=args.use_raw_train)
        test_dataset = MVSDataset(args.testpath, args.testlist, "val", args.n_views, args.interval_scale)

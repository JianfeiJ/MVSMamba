...
parser.add_argument('--model', default='mvsmamba', help='select model')

parser.add_argument('--dataset', default='general_eval4', help='select dataset')
parser.add_argument('--testpath', default="/space/jiangjf/dtu_test/", help='testing data dir for some scenes')
parser.add_argument('--testlist', default="lists/dtu/test.txt", help='testing scene list')

parser.add_argument('--batch_size', type=int, default=1, help='testing batch size')

# parser.add_argument('--loadckpt', default='./checkpoints/aa_final_version_neurips_dtu_raw/a_final_neurips_e9/finalmodel_9.ckpt', help='load a specific checkpoint')
parser.add_argument('--loadckpt', default='./checkpoints/mamba_reg_1652_no_last_pixel_raw_b2/finalmodel_6.ckpt', help='load a specific checkpoint')
# parser.add_argument('--loadckpt', default='./checkpoints/base_321688/finalmodel_15.ckpt', help='load a specific checkpoint')

parser.add_argument('--outdir', default='./outputs/mamba_reg_1652_no_last_pixel_raw_b2', help='output dir')

parser.add_argument('--share_cr', action='store_true', help='whether share the cost volume regularization')

parser.add_argument('--ndepths', type=str, default="32,16,8,4", help='ndepths')
parser.add_argument('--depth_inter_r', type=str, default="2,1,1,0.5", help='depth_intervals_ratio')
...
parser.add_argument('--group_cor_dim', type=str, default="4,4,4,4", help='group correlation dim')
...
parser.add_argument('--fpn_mamba', default=True, help='mamba for multi-view feature enhancement')
...
def reproject_with_depth_open3d(depth_ref, intrinsics_ref, extrinsics_ref, depth_src, intrinsics_src, extrinsics_src):
    width, height = depth_ref.shape[1], depth_ref.shape[0]
    ## step1. project reference pixels to the source view
    # reference view x, y
    x_ref, y_ref = np.meshgrid(np.arange(0, width), np.arange(0, height))
    ...
    xyz_ref_space = np.matmul(np.linalg.inv(extrinsics_ref),
                              np.vstack((xyz_ref, np.ones_like(x_ref))))[:3]
    xyz_ref_space = xyz_ref_space.reshape([3, height, width]).astype(np.float32)
    # source 3D space
    xyz_src = np.matmul(np.matmul(extrinsics_src, np.linalg.inv(extrinsics_ref)),
                        np.vstack((xyz_ref, np.ones_like(x_ref))))[:3]
    # source view x, y
    ...
    xyz_src = np.matmul(np.linalg.inv(intrinsics_src),
                        np.vstack((xy_src, np.ones_like(x_ref))) * sampled_depth_src.reshape([-1]))
    # reference 3D space
    xyz_reprojected = np.matmul(np.linalg.inv(extrinsics_src),
                                np.vstack((xyz_src, np.ones_like(x_ref))))[:3]
    # source view x, y, depth
    xyz_src_space = xyz_reprojected.reshape([3, height, width]).astype(np.float32)


    return xyz_ref_space, xyz_src_space  # ,conf
...
def filter_depth(pair_folder, scan_folder, out_folder, plyfilename):
    ...
    for ref_view, src_views in pair_data:
        ...
        # print(ref_depth_est.shape,confidence.shape)
        ...
        print("processing {}, ref-view{:0>2}, photo/geo/geo_sum/final-mask:{}/{}/{}/{}".format(scan_folder, ref_view,
                                                                                               photo_mask.mean(),
                                                                                               geo_mask.mean(),
                                                                                               geo_mask_sum.mean(),
                                                                                               final_mask.mean()))
...
def dypcd_filter(testlist, number_worker=4):
    # for scene in testlist:
    #     dypcd_filter_worker(scene)
...
def visualize_depth(depth, mask=None, depth_min=None, depth_max=None, direct=False):
    ...
    depth_color = cv2.applyColorMap(depth_scaled_uint8, cv2.COLORMAP_PLASMA)
...
def save_scene_depth(testlist):
    ...
    # model
    model = MVSMamba(arch_mode=args.arch_mode, num_stage=4,
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            for filename, cam, img, depth_est, photometric_confidence in zip(filenames, cams, imgs, \
                ...
                cv2.imwrite(pred_depth_img_path, depth_color)

                # save confidence maps
                save_pfm(confidence_filename, photometric_confidence)
                cv2.imwrite(confidence_img_filename, visualize_depth(photometric_confidence))

...
d_r = {'Auditorium':1.0, 'Ballroom':1.0, 'Courtroom':1.0, 'Museum':1.0, 'Palace':1.0, 'Temple':1.0}
r_r = d_r
# Filter hyperparameter Settings
##########################################

# nips_v1
s_all = {'Auditorium':2, 'Ballroom':3, 'Courtroom':2, 'Museum':3, 'Palace':2, 'Temple':2}
conf_all = {'Auditorium':0.01, 'Ballroom':0.01, 'Courtroom':0.05, 'Museum':0.05, 'Palace':0.05, 'Temple':0.05}
dist_all = {'Auditorium':1/1.5*d_r['Auditorium'], 'Ballroom':1/1.5*d_r['Ballroom'], 'Courtroom':1/1.5*d_r['Courtroom'],
            'Museum':1/1*d_r['Museum'], 'Palace':1/1*d_r['Palace'], 'Temple':1/1*d_r['Temple']}
rel_diff_all = {'Auditorium':1/350*r_r['Auditorium'], 'Ballroom':1/1200*r_r['Ballroom'], 'Courtroom':1/1200*r_r['Courtroom'],
                'Museum':1/1200*r_r['Museum'], 'Palace':1/1200*r_r['Palace'], 'Temple':1/1200*r_r['Temple']}
...
parser.add_argument('--testpath', default='/space0/jiangjf/Data/tanksandtemples_1/advanced/', help='testing data dir for some scenes')
parser.add_argument('--testlist', default='lists/tnt/adv.txt', help='testing scene list')

parser.add_argument('--batch_size', type=int, default=1, help='testing batch size')

parser.add_argument('--loadckpt', default='checkpoints/bld/final_version_neurips_dtu_low_bld_low/dtu_low_bld_low_crop_b2lr05n11_768x576/finalmodel_12.ckpt', help='load a specific checkpoint')
parser.add_argument('--outdir', default='./outputs/test', help='output dir')
...
parser.add_argument('--num_view', type=int, default=21, help='num of view')
parser.add_argument('--fusion_view', type=int, default=11, help='num of view')
...
def check_geometric_consistency(depth_ref, intrinsics_ref, extrinsics_ref, depth_src, intrinsics_src, extrinsics_src,scan):
    ...
    for i in range(s,args.fusion_view):
        mask = np.logical_and(dist < i * dist_base, depth_diff < i * rel_diff_base)

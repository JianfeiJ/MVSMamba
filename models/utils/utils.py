...
class GLU_3D(nn.Module):
    def __init__(self, dim, mid_dim):
        super(GLU_3D, self).__init__()
        self.W = nn.Conv3d(dim, mid_dim, kernel_size=3, padding=1, bias=False)
        self.V = nn.Conv3d(dim, mid_dim, kernel_size=3, padding=1, bias=False)
        self.W2 = nn.Conv3d(mid_dim, dim, kernel_size=3, padding=1, bias=False)
        self.act = nn.GELU()
    ...
    def forward(self, feat):
        ...
        Returns:
            (B, C, D, H, W)
        """
        feat_act = self.act(self.W(feat))     # (B, mid_dim, D, H, W)
        feat_linear = self.V(feat)            # (B, mid_dim, D, H, W)
        feat = feat_act * feat_linear         # GLU操作
        feat = self.W2(feat)                  # (B, C, D, H, W)
        return feat

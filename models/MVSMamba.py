...
# from .mamba_cost3d import FeatMamba2D


class MVSMamba(nn.Module):
    def __init__(self, arch_mode="fpn", num_stage=4, fpn_base_channel=8,
                reg_channel=8, stage_splits=[8,8,4,4], depth_interals_ratio=[0.5,0.5,0.5,0.5],
                group_cor=True, group_cor_dim=[4,4,4,4],
                inverse_depth=True,
    ...
    def forward(self, imgs, proj_matrices, depth_values):
        ...
        if self.fpn_mamba:
            ...
            for nview_idx in range(V):  # imgs shape (B, N, C, H, W)
                img = imgs[nview_idx]
